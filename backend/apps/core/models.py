...
class TenantScopedModel(UUIDPrimaryKeyModel):
    """Base for every Layer 1/2 model. `institution_id` is a plain indexed
    UUIDField, not a ForeignKey — Django cannot enforce FK constraints
    across the database boundary dedicated-DB-tier tenants introduce, so
    this is applied uniformly rather than varying by isolation tier
    (docs/multitenancy.md §1)."""
